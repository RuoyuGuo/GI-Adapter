...
class CLIPVisionTransformer(nn.Module):
    ...
    def get_all_features(self, x: torch.Tensor, use_adapter=True, verbose=False, get_delta=False):
        if verbose:
        ...
        B, C, H, W = x.shape
        if verbose:
            print('This is after the first conv1', B, C, H, W)
        x = x.reshape(x.shape[0], x.shape[1], -1) 
        x = x.permute(0, 2, 1)
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)
        ...
        x = x.permute(1, 0, 2)
        if verbose:
            print('After the first CNN + some processing', x.shape)
        ...
        for i, blk in enumerate(self.transformer.resblocks):
            ...
            # if drop_vlm[i]:
            #     x = random_token_drop_vlm(x, blk, ratio)
            # else:
            x = blk(x)
                # x = x.permute(1,0,2)
                
            # if drop_vfm[i]:
            #     dinov2_x = random_token_drop_vfm(dinov2_x, self.dinov2.blocks[i], ratio)
            # else:
            ...
            if get_delta:
                vlm_features[i] = dinov2_eva[:,:dinov2_x.shape[1]].contiguous()[:,1:]
                vfm_features[i] = dinov2_eva[:,dinov2_x.shape[1]:].contiguous()[:,1:]
            else:
                vlm_features[i] = x.permute(1,0,2)[:,1:].contiguous()
                vfm_features[i] = dinov2_x[:,1:].contiguous()
            
                # analysis_feataure_adapter[i] = dinov2_eva
        ...
        # return before_apt_vlm, before_apt_vfm , after_apt_vlm, after_apt_vfm
        return analysis_feataure_vlm, analysis_feataure_vfm, None, None
    ...
    def forward(self, x: torch.Tensor, use_adapter=True):
        ...
        # print('This is real forward, image shape coming with:', x.shape)
        # print('This is real forward, image shape coming with:', x.shape)
        # print('This is real forward, image shape coming with:', x.shape)
        # print('This is real forward, image shape coming with:', x.shape)
        # print('This is real forward, image shape coming with:', x.shape)
