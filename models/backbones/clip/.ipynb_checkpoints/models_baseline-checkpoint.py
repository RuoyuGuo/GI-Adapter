...
class CLIPVisionTransformerBASELINE(nn.Module):
    ...
    def foward_with_mask(self, x: torch.Tensor, drop_vlm, drop_vfm, ratio=0.5, mask_gene='random'):
        # Various Foundation Models use different normalization, convert inputs correspondingly
        IMG_MEAN = torch.tensor([ v*255 for v in [0.48145466, 0.4578275, 0.40821073]]).view(1, 3, 1, 1).cuda()
        IMG_STD = torch.tensor([ v*255 for v in [0.26862954, 0.26130258, 0.27577711]]).view(1, 3, 1, 1).cuda()
        original_x = x * IMG_STD + IMG_MEAN
        ...
        for i, blk in enumerate(self.transformer.resblocks):
            ...
            if mask_gene == 'random':
                mask1 = token_generate(x, ratio) 
                mask2 = token_generate(x, ratio)
            if mask_gene == 'same':
                mask1 = token_generate(x, ratio) 
                mask2 = mask1
            if mask_gene == 'comple':
            ...
            if drop_vlm[i]:
                x = random_token_drop_vlm(x, blk, mask1, ratio)
            else:
                x = blk(x)
                # x = x.permute(1,0,2)
                
            if drop_vfm[i]:
                dinov2_x = random_token_drop_vfm(dinov2_x, self.dinov2.blocks[i], mask2, ratio)
            else:
                dinov2_x = self.dinov2.blocks[i](dinov2_x)   
