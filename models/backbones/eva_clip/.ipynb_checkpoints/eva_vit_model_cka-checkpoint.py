...
class EVAVisionTransformerCKA(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.vlm_adapter   = nn.Sequential(*[MSGateAdapter(borrow_flag=vlm_flag[i]) for i in range(24)]) 
    ...
    def extract_feats(self, x, use_fpn=True, use_adapter=True, train_loss=False):
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x_delta       = self.vlm_adapter[i](x_self=x[:,1:], x_borrow=dinov2_x[:,1:])
            dinov2_delta  = self.vfm_adapter[i](x_self=dinov2_x[:,1:],         x_borrow=x[:,1:])
