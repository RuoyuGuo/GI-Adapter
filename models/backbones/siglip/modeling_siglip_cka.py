...
class SiglipVisionTransformerCKA(nn.Module):
    def __init__(self, hidden_size=768, intermediate_size=3072, num_hidden_layers=12, num_attention_heads=12, num_channels=3,
        ...
        out_indices=[7, 11, 15, 23], pretrained=None, output_attentions=False, output_hidden_states=False, return_dict=True, 
        #myparams
        adapter_type=None,
        **kwargs,):
        super().__init__()
    ...
    def forward(
        ...
        for i, encoder_layer in enumerate(self.encoder.layers):
            ...
            hidden_states_delta = self.vlm_adapter[i](x_self=hidden_states  , x_borrow=dinov2_x[:,1:])
            dinov2_delta        = self.vfm_adapter[i](x_self=dinov2_x[:,1:] , x_borrow=hidden_states)

            #
            hidden_states  = hidden_states  + hidden_states_delta
        ...
        #after loop
        # vlm_feature = self.convert_list_to_tensor(vlm_feature_list) # remove cls token
        # vfm_feature = self.convert_list_to_tensor(vfm_feature_list)[:, :, 1:, :] # remove cls token
