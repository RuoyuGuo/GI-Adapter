...
class tqdmHead(BaseDecodeHead):
    ...
    def myforward_test(self, inputs, texts, img_metas, test_cfg):
        """Test segment without test-time aumengtation.
        ...
        Returns:
            seg_mask (Tensor): Predicted semantic segmentation logits.
        """
        all_cls_scores, all_mask_preds, feature = self(inputs, texts, img_metas, return_mask_features=True)
        cls_score, mask_pred = all_cls_scores[-1], all_mask_preds[-1]
        ori_h, ori_w, _ = img_metas[0]['ori_shape']
        ...
        mask_pred = mask_pred.sigmoid()
        seg_mask = torch.einsum('bqc,bqhw->bchw', cls_score, mask_pred)
        return seg_mask, mask_pred, feature


    def forward_with_mask(self, inputs, texts, img_metas, test_cfg):
        """Test segment without test-time aumengtation.
